...
**Notes:** `src/app/core/models/journal-entry.model.ts`.
...
**Depends on:** HN-3
**Notes:** `src/app/core/services/entry-storage.service.ts`, native `indexedDB` (no wrapper library), db `halanaplo` / store `entries` keyed by date. Unit-tested against `fake-indexeddb` (see `src/test-setup.ts`).
...
**Notes:** Built into `src/app/shared/entry-form/entry-form.ts` (`.mood-grid`), matching `design/code.html`'s `#mood-selector` layout. Persists on explicit Save, along with the 3 items (see HN-6 — same form, same save action).
...
**Depends on:** HN-3, HN-4
**Notes:** Uniform 3 textareas (`entry-form.html`), styled via the HN-15 tokens. Save behavior decided: **explicit Save button** (not autosave) — one button saves mood + all 3 items together for the form's date; see HN-7 for how unsaved changes are protected.
...
**Depends on:** HN-6
**Notes:** Partial saves work (no validation blocks a save with fewer than 3 items). "X/3 kész" badge in `entry-form.html`. Since save is explicit (not autosave), "no entry silently lost" is handled by `unsavedChangesGuard` (`src/app/core/guards/unsaved-changes.guard.ts`) — a `CanDeactivate` guard that prompts before leaving `/today` or a day-detail page with unsaved changes.
...
**Notes:** `src/app/calendar/calendar.ts`, Monday-start Hungarian week. Future days render dimmed and non-interactive (no link).
...
**Depends on:** HN-8
**Notes:** `/calendar/:date` → `src/app/calendar/day-detail/day-detail.ts`, guarded by `validDateGuard` (redirects to `/calendar` for future/invalid dates). Past entries are **editable**, reusing `EntryForm` — decided alongside HN-6/HN-7 (explicit Save, past days editable).
...
**Depends on:** HN-1
**Notes:** `manifest.webmanifest` has `name`/`short_name`/`theme_color`/`background_color`/`description`; `<meta name="theme-color">` added to `index.html`. Verified by serving the production build statically (`npx http-server dist/halanaplo/browser`) and confirming `navigator.serviceWorker.getRegistrations()` shows `ngsw-worker.js` active and `/manifest.webmanifest` resolves with all 8 icons. Did **not** run a full offline network-cut test or a Lighthouse installability audit — worth a manual check on a real device before relying on it.
...
**Depends on:** HN-10
**Notes:** Simple mark (cream heart on the primary terracotta, rounded square) generated from `scripts/icon-source.svg` via `scripts/generate-icons.mjs` (uses `sharp`, installed on demand — not a project dependency). Regenerate after any palette/logo change. `name`/`short_name` are both "Hálanapló" (fits without truncation on a phone home screen).
...
**Notes:** All tap targets (`nav-btn`, back links, mood buttons, save button) are ≥44px; `env(safe-area-inset-top/bottom)` applied in `app.scss`, `entry-form.scss`'s toast. Layout is fluid (flex/grid, relative units, no fixed pixel widths beyond icon sizes), so it should scale correctly at any phone width. **Not independently verified** at multiple exact viewport widths — the browser automation tool's window resize didn't take effect in this environment (viewport stayed at the display's native size regardless of the requested size), so narrow-width screenshots weren't possible here. Worth a quick manual check in real dev-tools device emulation or on a phone.
...
**Depends on:** HN-4
**Notes:** `src/app/core/services/export-import.service.ts` (`exportToFile`), reachable from `/settings`. Pretty-printed JSON `{ exportedAt, entries[] }`, downloaded via a Blob URL.
...
**Depends on:** HN-13
**Notes:** `ExportImportService.importFromFile` — conflict policy is **overwrite** (imported entries replace any existing local entry for the same date), since restoring from a backup is the primary use case. Accepts either the `{ entries: [...] }` wrapper or a bare array.
...
**Depends on:** HN-15
**Notes:** `src/app/core/util/greeting.ts` + `Today` component (`src/app/today/today.ts`), date formatted via `DatePipe` with the `hu` locale (registered in `app.config.ts`).
...
**Notes:** `src/app/core/util/streak.ts` (`computeStreak`) — consecutive days ending today (or yesterday if today has no entry yet) with at least one filled gratitude item. Recomputed on every save.
...
**Depends on:** HN-15
**Notes:** `src/app/settings/settings.ts`, linked from a gear icon in the app header (`app.html`). Content is the HN-13/HN-14 export/import UI.
